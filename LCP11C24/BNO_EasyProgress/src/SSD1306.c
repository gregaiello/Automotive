 ...
 * | SSD1306 libs for LPC based on the (amazing) code by Tilen Majerle
...
void support_routine_I2C_Write_command(uint8_t reg, uint8_t command){
  ...
  config[0] = reg;
  config[1] = command;
  Chip_I2C_MasterSend(I2C0, SSD1306_I2C_ADDR, config, I2C_DATA_LEN);
}
...
void configure_SSD1306(){
  ...
  SSD1306_Puts("CLIMER", &Font_16x26, SSD1306_COLOR_BLACK);
  SSD1306_UpdateOLED();
...
void write_multiple_SSD1306(uint8_t* data, uint16_t count){ // count = SSD1306_WIDTH
	uint8_t *reg;
	reg = (((uint8_t *)data)-1);
	*reg = 0x40;
	Chip_I2C_MasterSend(I2C0, SSD1306_I2C_ADDR, (data-1), SSD1306_WIDTH +1);
